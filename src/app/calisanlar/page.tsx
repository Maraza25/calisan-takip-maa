...
import { UserPlus, Search, UserX, UserCheck, AlertCircle } from 'lucide-react';
import {
  getAllEmployees,
  addEmployee,
  getEmployeeByTC,
...
export default function EmployeesPage() {
  ...
  }, [searchTerm, employees, performSearch]);
  ...
  const handleAddEmployee = async (e: React.FormEvent) => {
    ...
    try {
      // Aynı TC'ye sahip çalışan var mı kontrol et
      const existing = await getEmployeeByTC(tc);

      if (existing) {
        if (!existing.disabled) {
          setFormError('Bu TC kimlik numarasına sahip aktif bir çalışan zaten var.');
          setFormLoading(false);
          return;
        } else {
          // Pasif çalışan var, reactivation modal göster
      ...
      // Yeni çalışan ekle
      await addEmployee(tc, fullName.trim());
        ...
        {existingEmployee && (
          <div className="space-y-4">
            <p className="text-gray-600 dark:text-gray-400">
              <strong>{existingEmployee.fullName}</strong> adlı çalışan 
              bu TC kimlik numarasıyla geçmişte kayıtlı ancak şu anda pasif durumda. 
              Bu kişiyi tekrar aktif etmek ister misiniz?
            </p>
